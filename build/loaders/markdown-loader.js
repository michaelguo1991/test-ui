...
const util = require('../markdown/util');

module.exports = function(src) {
    const { markdown } = getOptions(this);

    // const { html, data: { hoistedTags } } = markdown.render(src);

    const content = markdown.render(src);
    const html = util.stripTags(content, ['script', 'style']);
    const script = util.fetchTag(content, 'script');
    const style = util.fetchTag(content, 'style');

    let result = `<template>\n` +
        `<div class="content">${html}</div>\n` +
    `</template>\n`;
    script && (result += `<script>${script}</script>\n`);
    style && (result += `<style>${style}</style>\n`);
